...
import { Bevel, TitleBar, RADIUS, WELL } from '../ui';

function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section style={{ marginBottom: 15 }}>
      <h3 style={{ font: '700 13px/1 var(--font-sans)', color: '#2a4a8a', marginBottom: 5 }}>{title}</h3>
      <div style={{ font: '400 13px/1.6 var(--font-sans)', color: '#1c1a17' }}>{children}</div>
    </section>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, padding: '40px 12px 12px', display: 'flex' }}>
      <Bevel style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, padding: 3 }}>
        <TitleBar title="About Classicverse" />
        <div style={{
          flex: 1, minHeight: 0, overflowY: 'auto', background: '#fff',
          borderRadius: `0 0 ${RADIUS}px ${RADIUS}px`, boxShadow: WELL, padding: '16px 18px',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 14 }}>
            <span style={{ display: 'inline-flex', borderRadius: 3, overflow: 'hidden', boxShadow: '0 0 0 1px rgba(0,0,0,0.4)' }}>
              {['#9a2a2a', '#d4a017', '#1f6f3e', '#2a4a8a'].map((c) => (
                <span key={c} style={{ width: 11, height: 24, background: c, display: 'block' }} />
              ))}
            </span>
            <h2 style={{ font: '800 24px/1 var(--font-sans)', color: '#1c1a17' }}>Classicverse</h2>
          </div>
          ...
          <Section title="What's inside">
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li style={{ marginBottom: 4 }}><strong>A century of cars</strong> — {CARS.length} cars, one per year, grouped by decade, each with sources and the reasoning for its selection.</li>
              <li style={{ marginBottom: 4 }}><strong>F1 Archive</strong> — all {FERRARI_WINS.length} of Ferrari&apos;s Grand Prix victories, each with a photograph of the car that scored it.</li>
              <li style={{ marginBottom: 4 }}><strong>Radio</strong> — {RADIO_STATIONS.length} stations, {tracks} recordings, about {minutes} minutes of period music.</li>
          ...
          <Section title="How it's built">
            ...
            button clicks to the CRT powering up, is synthesised in the browser; none of it is a file.
          </Section>
          ...
          <p style={{ font: '400 12px/1.5 var(--font-sans)', color: '#5a554d', marginTop: 4 }}>
