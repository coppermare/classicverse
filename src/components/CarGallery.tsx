'use client';

import { useEffect, useRef } from 'react';
import type { CarRecord } from '@/types/car';

interface CarGalleryProps {
  cars: CarRecord[];
  selectedYear: number;
  onOpen: (year: number) => void;
  onHighlight?: (year: number) => void;
}

// Full image gallery for the century of cars — a scrollable grid of every year's
// hero car. Clicking a tile opens that car full-screen (the cars view, which has
// its own prev/next navigation). The roller/keyboard highlight is mirrored here.
export default function CarGallery({ cars, selectedYear, onOpen, onHighlight }: CarGalleryProps) {
  const sorted = [...cars].sort((a, b) => a.year - b.year);
  const activeRef = useRef<HTMLButtonElement>(null);

  useEffect(() => {
    activeRef.current?.scrollIntoView({ block: 'nearest' });
  }, [selectedYear]);

  return (
    <div
      style={{
        position: 'absolute', inset: 0, overflowY: 'auto',
        background: '#14110d', paddingTop: 44,
        fontFamily: 'var(--font-sans)',
      }}
    >
      <div
        style={{
          display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)',
          gap: 0, padding: 0,
        }}
      >
        {sorted.map((car) => {
          const active = car.year === selectedYear;
          return (
            <button
              key={car.year}
              ref={active ? activeRef : undefined}
              type="button"
              onClick={() => onOpen(car.year)}
              onMouseEnter={() => onHighlight?.(car.year)}
              aria-label={`${car.year} — ${car.hero_car_name}`}
              aria-current={active ? 'true' : undefined}
              style={{
                position: 'relative', display: 'block', padding: 0, cursor: 'pointer',
                border: 'none', borderRadius: 0, overflow: 'hidden', aspectRatio: '4 / 3',
                background: '#1a1612',
                boxShadow: active
                  ? 'inset 0 0 0 2px #d4a017'
                  : 'inset 0 0 0 1px rgba(255,255,255,0.08)',
              }}
            >
              {car.image_url ? (
                /* eslint-disable-next-line @next/next/no-img-element */
                <img
                  src={car.image_url}
                  alt=""
                  loading="lazy"
                  style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
                />
              ) : (
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6a6258', fontSize: 13 }}>
                  {car.year}
                </div>
              )}

              <div
                style={{
                  position: 'absolute', left: 0, right: 0, bottom: 0,
                  padding: '14px 8px 5px', textAlign: 'left',
                  background: 'linear-gradient(180deg, transparent, rgba(0,0,0,0.82))',
                }}
              >
                <div style={{ fontSize: 11, fontWeight: 700, color: '#fff', lineHeight: 1.15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {car.hero_car_name}
                </div>
                <div style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.7)', letterSpacing: '0.04em' }}>
                  {car.year}
                </div>
              </div>
            </button>
          );
        })}
      </div>
    </div>
  );
}
